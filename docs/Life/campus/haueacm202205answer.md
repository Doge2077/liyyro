...
众所周知，$HF$ 最近天天泡在实验室里做他的智能小车车，但在调试的时候发现控制转向和行进的指令搞混了。这种小事对他来说太简单了，用他的原话说就是：“有手就行”，于是他就懒得继续做下去了。
...
$LYS$ 非常想要这个小车车，可是他啥也不会，现在他来找你帮忙了，你能帮帮他吗？
...
野兽仙贝拿出了三个超大号皮搋子放在桌上，分别编号为 $1,2,3$，初始对应位置编号也为 $1,2,3$。其中某个皮搋子下面藏着一个扩音器。
...
* 思维题。
  * 设位置数组为 `pos[N]`，初始时位置 `i` 上的卡片编号为 `i`，即 `pos[1] = 1` 表示位置 $1$ 上的卡片编号为 $1$。
  * 设答案数组为 `ans[N]`，表示编号为 `i` 的卡片被猜中多少次。
  * 用 `swap()` 交换位置上的卡片，而询问的位置上的卡片对应的编号被猜中，记录次数。
  * 最后比较哪个编号的卡片在交换时被猜中的次数最多，那么一开始答案就在这张卡片下。
...
我们的好姐姐 $WJX$ 在拼三角形的时候突发奇想，想要考考小陈同学。假如有 $n$ 个整数，以这 $n$ 个整数中的三个整数为边长能够构成多少个三角形？
...
现在给出一个“卷点”的定义：在一个正方形内存在某一点，使得这个点和正方形四个顶点连接而划分出的四块三角形的面积比为 $a:b:c:d$。四个数不分顺序；卷点可能不是唯一的。
...
![现在是摸鱼时间](https://cdn.jsdelivr.net/gh/Doge2077/liyyro-photo@main/images/2022/10/现在是摸鱼时间.png)

$LYS$ 最喜欢摸鱼了，现在给出一个长度为 $N$ 的序列 $A={A_1,A_2,\dots,A_N}$，表示 $LYS$ 的日程表，其中 $A_i$ 表示第 $i$ 天的摸鱼指数。

为了在排满的日程表里的摸鱼，他向摸鱼大师 $HF$ 请教，摸鱼大师给出了一个摸鱼公式：

* $\frac{1}{A_i} - \frac{j}{A_iA_j} = \frac{1}{A_j} - \frac{i}{A_iA_j},(i&lt;j)$
...
* 思维。
  * 化简 $\frac{1}{A_i} - \frac{j}{A_iA_j} = \frac{1}{A_j} - \frac{i}{A_iA_j},(i&lt;j)$ 得 $A_j - j = A_i - i$。
  * 由此，不妨设 $B_i = A_i - i$。
    ...
    for (auto &p : a) {
        for (auto &t : a) {
            if (gcd(p.first, t.first) == 1) {
                res = max(res, p.second + t.second);  // 更新答案
            }
        }
    }
    cout << res << endl;
