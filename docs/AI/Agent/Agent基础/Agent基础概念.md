...
| 专题 | 入口文章 | 适合解决的问题 |
| --- | --- | --- |
| Agent 架构 | [ReAct范式](../Agent架构/ReAct范式) | 理解推理、行动、观察的循环 |
| 工具调用 | [Function Calling原理](../Agent工具调用/Function%20Calling原理) | 理解模型如何提出工具调用请求 |
| 协议 | [MCP组成部分](../Agent协议专题/MCP组成部分) | 理解工具和上下文服务如何标准化接入 |
| 记忆 | [记忆类型](../Agent记忆/记忆类型) | 理解状态、经验和长期信息如何保存 |
| 协作 | [Single-Agent与Multi-Agent](../Agent协作/Single-Agent与Multi-Agent) | 理解单 Agent 到多 Agent 的选型 |
| 工程 | [手写AgentRuntime](../Agent工程/手写AgentRuntime) | 理解最小 Runtime 如何落地 |
| 评估 | [Agent评估总览](../Agent评估/Agent评估总览) | 理解任务、轨迹、评分和发布门禁 |

理解 Agent 可以从一次任务拆解开始：用户给出目标，Runtime 组织上下文，模型选择动作，工具返回观察，状态记录进展，系统判断是否继续。只要这条链路清楚，后续学习架构、工具、协议、记忆、协作、工程和评估都会更容易落到实现上。
