...
本文保留协议总览。MCP 和 A2A 的细节拆到独立专题：

| 文章 | 重点 |
| --- | --- |
| [MCP组成部分](../Agent协议专题/MCP组成部分) | Host、Client、Server、Tools、Resources、Prompts |
| [MCP最小实现](../Agent协议专题/MCP最小实现) | FastMCP Server、工具发现和资源读取 |
| [MCP传输机制](../Agent协议专题/MCP传输机制) | JSON-RPC、stdio、Streamable HTTP |
| [A2A与Agent间通信](../Agent协议专题/A2A与Agent间通信) | Agent Card、Task、Artifact 和状态流转 |
