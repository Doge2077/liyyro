...
对于 `templates`，`urls`，`views` 等的文件夹，最好都建立文件夹`menu`，`playground`，`settings` 文件夹，用于细分模块设计。
...
```
  ...
  * 一般来说，`static`存开发的文件，`media`存用户的文件。



同时要记得`import os`，`os.path()`是将路径合并在一起。通过该操作可以使我们的静态文件会放到这个目录下。
```java
