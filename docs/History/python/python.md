...
自从对着官方文档对着 `ipython` 敲了一遍，一天学完了 `Python` 的语法之后，我觉得我行了。于是屁颠屁颠地跑去写项目，结果显而易见，开发之路可谓寸步难行，一停下来就光速遗忘。
...
一般情况下无需显式调用 `__del__`，由解释器在进行垃圾回收时自动触发执行。
...
`__rtruediv__(self, other)` | `truediv` 反射除法，这个函数只有使用 `from __future__ import division` 时才有作用  
`__rmod__(self, other)` | `%` 反射取余操作符  
`__rdivmod__(self, other)` | 调用 `divmod(other, self)` 时 `divmod` 内建函数的操作  
`__rpow__(self, other)` | `**` 反射操作符  
...
`__imod__(self, other)` | 模赋值，相当于 `%=` 运算符  
...
`__ixor__(self, other)` | 异或赋值，相当于 `^=` 运算符  
...
`__trunc__(self)` | 当`math.trunc(self)`被调用时，调用`__trunc__`方法，返回自身类型的整型截取值  
...
`__setitem__(self, key, val)` | `self[key] = val` | 使用索引给某个元素赋值  
`__delitem__(self, key)` | `del self[key]` | 使用索引删除某个元素  
...
`__enter__(self)` | `with self as x:` | `with` 语句的上下文管理器  
`__exit__(self, exc, val, trace)` | `with self as x:` | `with` 语句的上下文管理器  
`__getstate__(self)` | `pickle.dump(pkl_file, self)` | `pickle` 序列化  
`__setstate__(self, state)` | `data = pickle.load(pkl_file)` | `pickle` 序列化