...
与常见的关系型数据库不同，`Neo4j` 基于图结构来表示和存储数据，并提供了声明式的查询语言 `Cypher` 来处理图数据。`Neo4j` 实现了专业数据库级别的图数据模型的存储，提供了完整的数据库特性，包括 `ACID` 事务的支持、集群的支持、备份和故障转移等。
...
这里为了便于练习，我将其使用 `Docker` 部署到了我的云服务器上，参考 [Docker部署](https://neo4j.com/docs/operations-manual/4.4/docker/)，其他部署方式[参考](https://neo4j.com/docs/operations-manual/4.4/)。
...
不出意外，便能看到如下界面，建议想要快速入门的话直接跟着官方教程走一遍：
...
* `Cypher` 与关系型数据库中的 `SQL`，一些关键词来源于 `SQL`，比如：`CREATE`、`WHERE`、`RETURN` 等。
  * `Cypher` 关键字大小写不敏感，也使用 `;` 分割多条查询语句。
...
* **属性**：
  ...
  * 节点的属性可以理解为关系型数据库中的字段。关系中的属性进一步明确了关系。
...
`Neo4j` 社区版不支持创建、删除和修改数据库，即上面的三个命令社区版用不了，只能使用默认的 `neo4j` 和 `system` 数据库。
...
也可以返回我们创建好的关系：
```cypher
CREATE (n:DOG {name: "LYS", age: 14}) -[r:IN_FAN]-> (m:CAT {name: "Hiiro", age: 17}) RETURN r;
...
CREATE (n:DOG {name: "LYS", age: 14}) -[:LOVER]-> (:BIRD {name: "Astesia", age: 13}) -[:FRIEND]-> (m:CAT {name: "Hiiro", age: 17}), (n) -[:FAN_OF]-> (m), (c:MOUSE {name: "ChenRay", age: 114}) -[:LOVER]-> (:DOG {name: "LexBurner", age: 514}), (c) -[:FAN_OF]-> (m), (n) -[:WORK_FOR]-> (p:PLATFORM {name: "BILIBILI"}) -[:HAVE]-> (:DOG {name: "UPs", age: 1919}), (p) -[:BELONGS_TO]-> (c);
...
例如，分页查询标签为 `DOG` 的节点信息，按照年龄从小到大排序，每页2条数据：
...
更新节点的属性和标签，例如将标签为 `DOG` 且年龄小于等于 60 的节点的标签更改为 `BOSS` 并将年龄增加 10：
```cypher
MATCH (n:DOG)
WHERE n.age <= 60
SET n:BOSS
