...
* 将整个哈希值空间组成一个虚拟的圆环，然后根据某种哈希算法将数据项映射到该圆环上。
  ...
  * 发生读写的 key 经过 hash 后，顺时针查找圆环上的节点，若未找到，则默认为 0 位置后的第一个节点。
...
public class AppController {
    ...
    private static final String LOCK = "TICKETSELLER";
...
public class AppController {
    ...
    @GetMapping("/ticket")
    public void sellTicket() {
        // 使用Lua脚本实现原子的加锁和设置过期时间
        String luaScript = "if redis.call('setnx', KEYS[1], ARGV[1]) == 1 then " +
                           "redis.call('expire', KEYS[1], ARGV[2]) " +
                           "return 1 else return 0 end";
        
        Long isLocked = stringRedisTemplate.execute(
...
**主要修复：**
1. 修正了代码格式和缩进，保持一致性
2. 将常量 `LOCK` 和 `KEY` 定义为 `private static final` 并放在类级别
3. 补全了 `@RestController` 和类定义
4. 在缺陷分析中，将“解决方案：”改为“#### 解决方案：”以保持格式一致
5. 补充了完整的 Lua 脚本示例，展示了如何实现原子操作
...
* 如果 B 的业务还没结束，A 的业务结束进行释放锁的操作，A 就会错误地删除掉 B 加的锁，那 B 的业务执行完就无锁可释了
  * 如果 B 服务可以获取到锁了，B 加锁并执行它的业务，由于此时 A 也在执行业务，两个服务共享内存就容易造成超卖问题
...
Redisson 内置了一系列的分布式对象，分布式集合，分布式锁，分布式服务等诸多功能特性，是一款基于 Redis 实现，拥有一系列分布式系统功能特性的工具包，是实现分布式系统架构中缓存中间件的最佳选择。
...
public class RedissonAppController {
    ...
    String LOCK = "REDISSON:TICKETSELLER";
    String KEY = "TICKET";
    ...
    public void redissonSellTicket() {
        ...
        try {
            ...
            if (count > 0) {
                ...
                System.out.println("Redisson get ticket");
            } else {
                System.out.println("Failed");
            }
        } catch (Exception e) {
...
**主要修改：**
1.  **错别字修正**：将 `(ExpirationEntry)EXPIRATION_RENEWAL_MAP.putIfAbsent` 中的多余类型转换移除。
2.  **格式优化**：
    *   统一了缩进风格（四个空格）。
    *   调整了代码块间的空行，使其结构更清晰。
    *   将注释移到了更合适的位置（`scheduleExpirationRenewal` 方法调用上方）。
3.  **命名优化**：为 `CompletionStage` 类型的变量赋予了更有意义的名称 (`stage`, `future`)，以提高代码可读性。
4.  **补全代码**：为 `scheduleExpirationRenewal` 方法的末尾补充了缺失的右花括号 `}`。
...
private void renewExpiration() {
    ...
    if (ee != null) {
        ...
        }, EXPIRATION_RENEWAL_MAP.get(getEntryName()).getTimeout(), TimeUnit.MILLISECONDS);
    }
}
...
以下是修复后的文本，已调整格式和缩进，并修正了部分标点符号的使用：
...
1. 统一了代码缩进格式（4空格）
2. 调整了注释表述，使其更符合中文习惯
3. 修正了字符串拼接语句的格式，使其更易读
4. 保持了代码逻辑不变，仅优化了可读性

// 计算超时时间，这是基于配置中的超时时间、重试间隔和重试次数计算出的总时间
...
-- 尝试从 KEYS[3] 获取值
local val = redis.call('get', KEYS[3])
-- 如果 val 不为 false，则返回它的数字表示
if val ~= false then
    return tonumber(val)
end
-- 检查 hash KEYS[1] 中是否存在字段 ARGV[3]
if (redis.call('hexists', KEYS[1], ARGV[3]) == 0) then
    -- 如果不存在，则返回 nil
    return nil
end
-- 在 hash KEYS[1] 里将字段 ARGV[3] 的值减 1，并将结果保存在 counter 中
local counter = redis.call('hincrby', KEYS[1], ARGV[3], -1)
-- 如果计数器仍然大于 0
if (counter > 0) then
    -- 设置 hash KEYS[1] 的过期时间为 ARGV[2] 毫秒
    redis.call('pexpire', KEYS[1], ARGV[2])
    -- 设置 KEYS[3] 的值为 0，并设置过期时间为 ARGV[5] 毫秒
    redis.call('set', KEYS[3], 0, 'px', ARGV[5])
    return 0
else
    -- 如果计数器不大于 0，则删除 hash KEYS[1]
    redis.call('del', KEYS[1])
    -- 执行 ARGV[4]（可能是发布某些消息）到 KEYS[2]
    redis.call(ARGV[4], KEYS[2], ARGV[1])
    -- 设置 KEYS[3] 的值为 1，并设置过期时间为 ARGV[5] 毫秒
    redis.call('set', KEYS[3], 1, 'px', ARGV[5])
    return 1
end
```
...
*   它是一个位图（bitmap），通过多个 hash 函数实现。
    ...
    *   如果一个请求的 key 经过多次映射后全部命中，则说明该 key 在 Redis 中**可能**存在，请求被放行。
    *   否则，请求被拒绝。

![image-20240320175505310](https://image.itbaima.cn/images/40/image-20240320174074053.png)

这里“全部命中”后仍然说“可能”存在，是因为存在哈希冲突的可能性：
*   假设已经缓存的 x, y, z 这三个 key 经过 hash 后命中的位图如上图所示。
    *   现在有一个新的 key `w` 请求到来，经过多次 hash 后发现其映射位置全部命中，这就造成了误判。

技术实现方式：
*   本地缓存：使用 Caffeine 或 Guava。
    *   分布式方案：基于 Redisson 实现。
...
这里主要是针对某个 key 的，某个 key 恰好过期并恰好有大量请求打到这个 key 上。
...
* **使用互斥锁：**
  ...
  * 设置成功后再查询 DB 并写回缓存，否则重试获取缓存。
  * 能保证强一致性，但线程需要等待，有死锁风险。
* **设置 key 逻辑过期：**
  * 设置 key 时添加一个字段表示逻辑过期时间，并设置该 key 永不过期。
  * 查询时比对逻辑过期时间判断是否过期。
  * 如果已过期，则新开线程进行数据同步，当前线程正常返回旧数据（数据不是最新的）。
  * 不能保证强一致性，但线程无需等待。
...
这算是缓存击穿的升级版，大量设置了相同过期时间的 key 同时失效，导致请求洪峰涌向数据库，虽然查询的 key 不同，但未命中的请求占了绝大部分。
