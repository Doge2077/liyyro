...
* 给定一组域：$D_1, D_2, D_3, \dots, D_n$，且允许其中某些域是相同的。
  * 则其笛卡尔积为：$D_1 \times D_2 \times \dots \times D_n = \{ (d_1, d_2, d_3, \dots, d_n) \mid d_i \in D_i, i = 1, 2, \dots , n \}$。
...
* 则笛卡尔积 $D_1 \times D_2 \times \dots \times D_n$ 的基数为：
  $$ M = \prod_{i=1}^{n} m_i $$
...
* 笛卡尔积的基数为 $2 \times 2 = 4$，共 $4$ 个元组，可生成表格如下：
...
* $D_1 \times D_2 \times \dots \times D_n$ 的子集叫做在域 $D_1, D_2, D_3, \dots, D_n$ 上的关系。
...
* 候选码（Candidate key）：若关系中的某一属性组的值能唯一地标识一个元组，而其任何子集都不能，则称该属性组为候选码（如学号，身份证号）。

* 全码（All key）：在最简单的情况下，候选码只包含一个属性。在最极端的情况下，关系模式的所有属性是这个关系模式的候选码。

* 主码（Primary key）：若一个关系有多个候选码，则选定其中一个为主码。候选码的诸属性称为主属性（Prime attribute）。不包含在任何候选码中的属性称为非码属性或非主属性（Non-key attribute）。
...
* 查询表：查询结果对应的表。
* 视图表：由基本表或其他视图表导出的表，是虚表，不对应于实际存储的数据。
...
* 若属性（或属性组）$F$ 是基本关系 $R$ 的外码，它与基本关系 $S$ 的主码 $K$ 相对应（基本关系 $R$ 和 $S$ 不一定是不同的关系），则对于 $R$ 中每个元组在 $F$ 上的值必须：
    * 取空值（$F$ 的每个属性值均为空值）
    * 或等于 $S$ 中某个元组的主码值。
...
* **笛卡儿积（Cartesian product）**：
  ...
  - 若 $R$ 有 $k_1$ 个元组，$S$ 有 $k_2$ 个元组，则关系 $R$ 和关系 $S$ 的笛卡儿积有 $k_1 \times k_2$ 个元组。记作
    $$
...
* $A$，$t[A]$，$\overline{A}$
  - 若 $A = \{ A_{i1}, A_{i2}, \dots, A_{ik} \}$，其中 $A_{i1}, A_{i2}, \dots, A_{ik}$ 是 $A_1, A_2, \dots, A_n$ 中的一部分，则 $A$ 被称为**属性列**或**属性组**。
...
- 选择运算符的含义：
  ...
  - 在基本的选择条件上可以进一步进行逻辑运算，求非（$\lnot$）、与（$\land$）、或（$\lor$）运算。

![选择](https://cdn.jsdelivr.net/gh/Doge2077/liyyro-photo@main/images/2023/02/选择.png)
...
![投影](https://cdn.jsdelivr.net/gh/Doge2077/liyyro-photo@main/images/2023/02/投影.png)
...
* **自然连接（natural join）**：
    ...
    * 自然连接的含义：
        * $R$ 和 $S$ 具有相同的属性组 $B$。
        * $$
          R \bowtie S = \{ \overset{\frown}{t_r t_s} [\overline{B}] \mid t_r \in R \land t_s \in S \land t_r[B] = t_s[B] \}
          $$
        * 其中 $\overline{B}$ 为去掉重复属性组 $B$ 后的剩余属性组。
