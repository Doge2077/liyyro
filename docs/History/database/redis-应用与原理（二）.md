...
Redis 是基于内存的数据库，数据存储在内存中，为了避免进程退出导致数据永久丢失，需要定期对内存中的数据以某种形式从内存中保存到磁盘当中；当 Redis 重启时，利用持久化文件实现数据恢复。
...
* 以日志的形式来记录用户请求的写操作，读操作不会记录，因为写操作才会存储数据  
  * 文件以追加的形式而不是修改的形式  
  * redis 的 AOF 恢复其实就是把追加的文件从开始到结尾读取并执行写操作  
...
* 首先创建一个不带有网络连接的伪客户端，此过程只需要载入 AOF 文件即可，因此无需网络连接  
...
例如上述命令在执行重写前，会记录 `list` 这个 `key` 的状态，重写前 AOF 要保存这五条命令，重写后只需要一条命令，结果却是等价的。  
...
* 手动：使用命令 `bgrewriteaof` ，如果当前有正在运行的 rewrite 子进程，则本次的重写会延迟执行，否则直接触发
...
* 文件大小：RDB 文件默认使用 LZF 算法进行压缩，压缩后的文件体积远远小于内存大小，适用于备份、全量复制等
  * 恢复速度：Redis 加载 RDB 文件恢复数据要远远快于 AOF 方式，AOF 则需要执行全部的命令
...
当开启混合持久化时，在 AOF 重写日志时，fork 出来的子进程会先将与主线程共享的内存数据以 RDB 的方式写入 AOF，然后主线程处理的操作命令会被记录在重写缓冲区里，重写缓冲区的增量命令会以 AOF 的方式写入 AOF 文件，写入完成后，通知主进程将新的含有 RDB 格式和 AOF 格式的 AOF 文件替换旧的 AOF 文件。

**注意**：对于采用混合持久化方案的 AOF 文件，AOF 文件的前半部分是 RDB 格式的全量数据，后半部分则是 AOF 格式的增量数据。

这样的好处在于，由于前半段为 RDB 格式的文件，恢复速度较快，加载完 RDB 的内容后再执行后半部分 AOF 的内容，以减少丢失数据的风险。
...
* `set &lt;key&gt; &lt;value&gt; ex &lt;n&gt;`：设置键值对的同时，指定过期时间（精确到秒）
  ...
  * `setex &lt;key&gt; &lt;n&gt; &lt;value&gt;`：设置键值对的同时，指定过期时间（精确到秒），该操作是一个原子操作
...
* 惰性删除：只有当客户端尝试获取一个 key 时，Redis 才会检查该 key 是否过期，如果过期则删除。
  * 定时删除：在设定 key 的过期时间的同时，创建一个定时器，当达到过期时间时，定时器立即删除该 key。
  * 定期删除：每隔一段时间随机取出一定数量的 key 进行检查，并删除其中已过期的 key。

Redis 实际采用的过期删除策略是：惰性删除 + 定期删除。
...
```c
/* The key needs to be converted from static to heap before it is deleted */
...
当 Redis 运行内存超过设置的最大内存时，会执行淘汰策略删除符合条件的 key，以保障高效运行。
...
* volatile-lfu：挑选最近最少频繁使用的数据淘汰（即使用次数最少）
* volatile-ttl：挑选将要过期的数据淘汰
* volatile-random：任意选择数据淘汰
* allkeys-lru：挑选最近最少使用的数据淘汰
* allkeys-lfu：挑选最近最少频繁使用的数据淘汰（即使用次数最少）
...
Redis 在每一个对象的结构体中添加了 `lru` 字段。根据内存淘汰策略的不同，它用于记录此数据最后一次被访问的时间戳（LRU策略）或访问频率（LFU策略），这里是基于全局 LRU 时钟或 LFU 计数器计算的。
...
robj *lookupKey(redisDb *db, robj *key, int flags) {
    ...
    if (de) {
        ...
        if (val && !isPausedActionsWithUpdate(PAUSE_ACTION_EXPIRE)) {
            if (server.masterhost && server.repl_replica_lazy_eviction &&
                (flags & LOOKUP_NOTOUCH)) {
                /* If the master client is this client, we can delay the
                 * eviction of the key. */
                if (server.current_client &&
                    (server.current_client->flags & CLIENT_MASTER)) {
                    /* Set the key to be expired after 5 seconds from now. */
                    setExpire(NULL, db, key, (mstime_t)5000 + server.mstime);
                } else {
                    /* Otherwise, just use the lookupKey to update the LRU or
                     * LFU. */
                    if (server.maxmemory_policy & MAXMEMORY_FLAG_LFU) {
                        updateLFU(val);
                    } else {
                ...
                /* If the key is not being kept in memory for replication, update
                 * the LRU or LFU. */
                if (server.maxmemory_policy & MAXMEMORY_FLAG_LFU) {
                    updateLFU(val);
                } else {
...
LFU 全称 Least Frequently Used，即“最近最不常用”。LFU 算法根据数据访问次数来淘汰数据，其核心思想是：“如果数据过去被访问多次，那么将来被访问的频率也更高。”

传统 LFU 算法实现：

* 基于链表结构，链表中的元素按照访问次数从大到小排序，新插入的元素放在尾部，访问后次数加一。
  * 当需要执行淘汰策略时，对链表排序，相同次数按照时间排序，删除访问次数最少的尾部元素。

Redis 实现的 LFU 算法是一种近似 LFU 算法。

---

首先，我们仍然从 Redis 源码中 `server.h` 对 `redisObject` 的定义入手：
...
在之前讲解 LRU 算法原理时，我仅提到 `lru` 字段作为 LRU 算法的时间戳使用。但如果选择 LFU 算法，该字段将被拆分为两部分：

* 低 8 位：计数器，初始值设为宏定义 `LFU_INIT_VAL = 5`。
* 高 16 位：以分钟为精度的 Unix 时间戳。

接下来仍然是 `db.c` 中的 `lookupKey` 函数，这次我们具体来看 LFU 的更新策略：
...
Redis 执行 LFU 淘汰策略和 LRU 基本一致，也是将所有待淘汰的 key 通过 `evictionPoolPopulate` 函数填入，区别在于填充策略的选择：
```c
```
...
如果从节点需求量较大，由于主从同步时，主节点需要发送自己的 RDB 文件给从节点进行同步，若此时从节点数量过多，主节点需要频繁地进行 RDB 操作，会影响主节点的性能。
...
* master 服务器会记录一个 replicationId 的伪随机字符串，用于标识当前的数据集版本，还会记录一个数据集的偏移量 offset。
* 主节点不断地把自己接收到的命令记录在 repl_backlog 中，并修改 offset。
* 执行增量同步时，主节点在 repl_backlog 中获取 offset 之后的数据并返回给从节点。
* 从节点接收数据后写入本地，修改 offset 与主节点一致。

**注意**：
* repl_backlog 大小有上限，超过后新数据会覆盖老数据。
* 如果从节点断开时间太久导致未备份的数据被覆盖，则无法基于 log 做增量同步。
...
因此引入 Sentinel，若主节点宕掉，则 Sentinel 会在从节点之间选举出一个节点作为主节点。
...
* Sentinel 基于心跳机制监控服务状态，每 1 秒向集群的每个实例发送一次 ping。
...
* 首先判断与主节点断开时间最短的从节点。
  * 若相同，则比较从节点的 `slave-priority` 值，值越小优先级越高。
  * 若仍相同，则比较 `offset` 值，值越大优先级越高。
  * 最后比较从节点运行 ID，ID 越小优先级越高。

### 脑裂问题

假设 Sentinel 和集群的各个实例处于不同的网络分区，由于网络抖动，Sentinel 未能感知到主节点，因此选举提升了一个从节点作为新的主节点：

* 客户端由于仍在向旧主节点写入数据。
  * 但网络恢复后，旧的主节点会被强制降为从节点，导致其原有数据丢失。

**解决方案**：

* 设置 Redis 参数：
  * `min-replicas-to-write 1`：表示最少的 slave 节点数量为 1。
  * `min-replicas-max-lag 5`：表示数据复制同步的延迟不能超过 5 秒。
  * 如果发生脑裂，原 master 会在客户端执行写入操作时拒绝请求，从而避免数据丢失。