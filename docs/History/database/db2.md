...
一个域允许的不同取值个数称为这个域的**基数** ：
  ...
  * 则 [latex]D_1 \times D_2 \times D_n \times \dots \times D_n[/latex] 的基数 [latex]M = \prod^{n}_{i = 1}m_i[/latex]。
...
设关系 $R$ 和关系 $S$ 具有相同的目（即两个关系都有 $n$ 个属性），且相应的属性取自同一个域，$t$ 是元组变量，$t \in R$ 表示 $t$ 是 $R$ 的一个元组。可以定义并、差、交、笛卡儿积运算如下：
  ...
  * 并（union）： 
    * 关系 $R$ 与关系 $S$ 的并记作： [latex]R \cup S = \\{t |t \in R \land t \in S \\}[/latex]
    * 其结果仍为 $n$ 目关系，由属于 $R$ 或属于 $S$ 的元组组成。
  * 差（except）： 
    * 关系 $R$ 与关系 $S$ 的差记作：[latex]R - S = \\{ t | t \in R \land t \notin S \\}[/latex]
    * 其结果关系仍为 $n$ 目关系，由属于 $R$ 而不属于 $S$ 的所有元组组成。
  * 交（intersection）： 
    * 关系 $R$ 与关系 $S$ 的交记作 [latex]R \cap S = \\{ t | t \in R \land t \in S \\}[/latex]
    * 其结果关系仍为 $n$ 目关系，由既属于 $R$ 又属于 $S$ 的元组组成。
    * 关系的交可以用差来表示，即 [latex]R \cap S = R - ( R - S )[/latex]。
  * 笛卡儿积（cartesian product）： 
    * 这里笛卡儿积的元素是元组。
    * 两个分别为 $n$ 目和 $m$ 目的关系 $R$ 和 $S$ 的笛卡儿积是一个 $(n + m)$ 列的元组的集合。元组的前 $n$ 列是关系 $R$ 的一个元组，后 $m$ 列是关系 $S$ 的一个元组。
    * 若 $R$ 有个元组，$S$ 有 $2$ 个元组，则关系 $R$ 和关系 $S$ 的笛卡儿积有 $k_1 \times k_2$ 个元组。记作 [latex]R \times S = \\{\overset{\frown}{t_r t_s} | t_r \in R \land t_s \in S \\}[/latex]
...
专门的关系运算包括选择、投影、连接、除运算等。为了叙述上的方便，先引入几个记号：
  ...
  * $A$，$t[A]$，$\overline{A}$ 
    * 若 [latex]A = \\{ A_{i1}, A_{i2}, \dots , A_{ik} \\}[/latex]，其中 [latex]A_{i1}, A_{i2}, \dots , A_{ik}[/latex] 是 [latex]A_1, A_2, \dots , A_n[/latex] 中的一部分，则 $A$ 被称为属性列或属性组。
    * [latex]t[A] = (t[A_{i1}], t[A_{i2}], \dots , t[A_{ik}])[/latex] 表示元组 $t$ 在属性列$A$ 上诸分量的集合。
    * [latex]\overline{A}[/latex] 则表示 [latex]\\{ A_1, A_2, \dots , A_n \\}[/latex] 中去掉 [latex]A_{i1}, A_{i2}, \dots , A_{ik}[/latex] 后剩余的属性组。
...
* * *
  ...
  * 选择运算符的含义： 
    * 在关系R中选择满足给定条件的诸元组
    * [latex]\sigma_F(R) = \\{ t | t \in R \land F(t) = `true' \\}[/latex]
...
* * *
  ...
  * 连接运算的含义： 
    ...
    * [latex]R \underset{A \theta B}{\bowtie} S = \\{ \overset{\frown}{t_r t_s} | t_r \in R \land t_s \in S \land t_r[A] \theta t_s[B] \\}[/latex]
...
两类常用连接运算：
  ...
  * 等值连接（equijoin）： 
    ...
    * [latex]R \underset{A = B}{\bowtie} S = \\{ \overset{\frown}{t_r t_s} | t_r \in R \land t_s \in S \land t_r[A] \theta t_s[B] \\}[/latex]
  * 自然连接（natural join）： 
    * 自然连接是一种特殊的等值连接：
    ...
    * 自然连接的含义：
    * $R$ 和 $S$ 具有相同的属性组 $B$
    * [latex]R \bowtie S = \\{ \overset{\frown}{t_r t_s} [U-B] | t_r \in R \land t_s \in S \land t_r[B] = t_s[B] \\}[/latex]
...
* * *
  ...
  * $P$ 是 $R$ 中满足下列条件的元组在 $X$ 属性列上的投影： 
    ...
    * [latex]R \div S = \\{ t_r[X] | t_r \in R \land \Pi_Y(S) \subseteq Y_x \\}[/latex]
