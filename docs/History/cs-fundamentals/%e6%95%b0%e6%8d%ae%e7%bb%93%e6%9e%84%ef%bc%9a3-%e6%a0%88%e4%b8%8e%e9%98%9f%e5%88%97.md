...
考查如下问题，任给十进制整数 `n`，将其转换为 `X` 进制的表示形式： [latex] \begin{aligned} n &= (d_m\dots d_2d_1d_0)_{(\lambda)}\\\\\\\ &= d_m\times\lambda^{m}+\dots d_2\times\lambda^{2} + d_1\times\lambda^{1} + d_0\times\lambda^{0}\\\\\\\ n_i &= (d_m\dots d_{i+1}\dots d_1)_{\lambda}\\\\\\\ d_i &= n_i\%\lambda\\\\\\\ n_{i+1} &= \lfloor\frac{n_i}{\lambda}\rfloor \end{aligned} [/latex]
