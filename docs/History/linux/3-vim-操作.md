...
*   命令行模式下的文本编辑器。
    ...
    *   使用方式：`vim filename`。如果已有该文件，则打开它。如果没有该文件，则打开一个新的文件，并命名为 `filename`。

**模式**：

*   一般命令模式：默认模式。命令输入方式：按下不同按键，即可进行不同操作。可以复制、粘贴、删除文本等。
    *   编辑模式：在一般命令模式里按下 `i`，会进入编辑模式。按下 `ESC` 会退出编辑模式，返回到一般命令模式。
    *   命令行模式：在一般命令模式里按下 `:`、`/`、`?` 三个按键中的任意一个，会进入命令行模式。命令行在最下面。可以查找、替换、保存、退出、配置编辑器等。
...
*   `n &lt;Space&gt;`：`n` 表示数字，按下数字后再按空格，光标会向右移动本行的 `n` 个字符。
