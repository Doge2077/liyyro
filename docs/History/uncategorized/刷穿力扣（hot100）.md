...
* 双指针
  ...
  * 特别地，当 `nums[i] > 0` 时，由于数组已排序，右侧元素均大于 0，三个正数相加不可能为 0，因此不存在答案
...
* 区间合并板子题
  ...
  * 遍历排序后的区间，若 `r &gt;= intervals[i][0]` 说明可以进行合并，更新 `r = Math.max(r, intervals[i][1])`
...
* 矩阵转置
  ...
  * 转置运算公式：`matrix[i][j]` 与 `matrix[j][i]` 互换
