...
1.  **错别字修正**：
    *   原注释 `// 每行的单词之间的间隙数量` 中的“间隙”已更正为正确的词语 **“间隔”**。在代码上下文中，它指的是单词之间的空格区域。

2.  **格式问题修正**：
    *   将 `for (int j = 0; j &lt; need; j ++)` 和 `for (int i = idx; i &lt; words.length; i ++)` 中的 `j ++` 和 `i ++` 修正为更符合通用编程规范的 `j++` 和 `i++`（运算符与变量间无空格）。
...
* **解法一：暴力枚举（模拟）**
    ...
    * 遍历矩阵的每一行，将每一行都看作一个柱状图的基底，利用 `heights` 数组计算以该行为底的最大矩形面积。对于 `heights[j]`：
    ```java
    * 设 cnt 为当前连续 1 的长度，res 为当前行从第 j 列向右，`heights[i][j]` 能保持的最小值。
    * 则最大矩形面积 `ans = max(ans, max(cnt, cnt * res))`
    ```
