...
[Original Link](https://codeforces.com/contest/1741/problem/A)
...
[Original Link](https://codeforces.com/contest/1741/problem/B)

**题目大意**：

- 给定一个 $1\sim N$ 的序列 $p$。
  - 求是否存在一种排列形式，使得：
    - 对于每个 $p_i$，至少存在一个相邻的元素使得它们之差不超过 $1$。
    - 且 $p_i \ne i$。
...
* 前缀和，模拟。
  ...
  * 当枚举到和大于 $t$ 的区间时，则进行剪枝。
  * 枚举结束，判断是否将所有元素都切分到了区间中，最后更新答案区间。
