...
**题目描述**
给定 `n` 个正整数 `a_i`，请你输出每个数的所有约数，每行一个数的约数，从小到大输出。
...
\vdots \\
p_k的约数之和 = p_k^{0}+p_k^{1}+\dots+p_k^{a_k} \\
\end{cases}
$$
...
$$
\begin{aligned}
\text{由样例可得增广矩阵：} \quad &\begin{pmatrix} 1 & 2 & -1 & -6 \\ 2 & 1 & -3 & -9 \\ -1 & -1 & 2 & 7 \end{pmatrix} \\\\

c_1\text{列中绝对值最大元素位于 } r_2\text{，将 } r_2 \text{ 与 } r_1 \text{ 交换：} \quad &\begin{pmatrix} 2 & 1 & -3 & -9 \\ 1 & 2 & -1 & -6 \\ -1 & -1 & 2 & 7 \end{pmatrix} \\\\

\text{将 } r_1 \text{ 的第 } c_1 \text{ 列元素变为 } 1\text{，需要使 } r_1 \times \frac{1}{2}： \quad &\begin{pmatrix} 1 & 0.5 & -1.5 & -4.5 \\ 1 & 2 & -1 & -6 \\ -1 & -1 & 2 & 7 \end{pmatrix} \\\\

\text{利用初等行变换将 } r_1 \text{ 下方的所有行的 } c_1 \text{ 列变为 } 0\text{：} \\
\text{即执行 } r_2 - r_1,\ r_3 + r_1： \quad &\begin{pmatrix} 1 & 0.5 & -1.5 & -4.5 \\ 0 & 1.5 & 0.5 & -1.5 \\ 0 & -0.5 & 0.5 & 2.5 \end{pmatrix} \\\\

\text{此时 } r_1 \text{ 已确定为阶梯矩阵的行，从 } r_1 \text{ 下方继续枚举，} \\
c_2\text{列中绝对值最大的元素位于 } r_2\text{，由于 } r_2 \text{ 上方无未确定的阶梯矩阵行，故不需要交换。} \\
\text{将 } r_2 \text{ 的第 } c_2 \text{ 列元素变为 } 1\text{，需要使 } r_2 \times \frac{2}{3}： \quad &\begin{pmatrix} 1 & 0.5 & -1.5 & -4.5 \\ 0 & 1 & \frac{1}{3} & -1 \\ 0 & -0.5 & 0.5 & 2.5 \end{pmatrix} \\\\

\text{利用初等行变换将 } r_2 \text{ 下方的所有行的 } c_2 \text{ 列变为 } 0\text{：} \\
\text{即执行 } r_3 + r_2 \times \frac{1}{2}： \quad &\begin{pmatrix} 1 & 0.5 & -1.5 & -4.5 \\ 0 & 1 & \frac{1}{3} & -1 \\ 0 & 0 & \frac{2}{3} & 2 \end{pmatrix} \\\\

\text{此时 } r_2 \text{ 已确定为阶梯矩阵的行，从 } r_2 \text{ 下方继续枚举，} \\
r_3\text{为最后一行，将 } r_3 \text{ 的第 } c_3 \text{ 列元素变为 } 1\text{，需要使 } r_3 \times \frac{3}{2}： \quad &\begin{pmatrix} 1 & 0.5 & -1.5 & -4.5 \\ 0 & 1 & \frac{1}{3} & -1 \\ 0 & 0 & 1 & 3 \end{pmatrix} \\\\

\text{判断该上三角矩阵中对角线元素均不为零，此时矩阵有唯一解，} \\
x_3 = 3\text{，从 } r_2 \text{ 进行回代：} \\
\text{即执行 } r_2 - r_3 \times \frac{1}{3}： \quad &\begin{pmatrix} 1 & 0.5 & -1.5 & -4.5 \\ 0 & 1 & 0 & -2 \\ 0 & 0 & 1 & 3 \end{pmatrix} \\\\

\text{此时可得 } x_2 = -2\text{，回代 } r_1\text{：} \\
\text{即执行 } r_1 - r_2 \times \frac{1}{2} + r_3 \times \frac{3}{2}： \quad &\begin{pmatrix} 1 & 0 & 0 & 1 \\ 0 & 1 & 0 & -2 \\ 0 & 0 & 1 & 3 \end{pmatrix} \\\\

\text{此时可得 } x_1 = 1\text{，综上可得：}
\begin{cases}
x_1 = 1 \\
x_2 = -2 \\
x_3 = 3
...
$1 \leq n \leq 10000,\quad 1 \leq b \leq a \leq 2000$
...
| 问题类型 | 修复说明 |
|---------|---------|
| 代码碎片化 | 将多个 `cpp`/`java` 碎片合并为完整程序 |
| 公式错误 | `Cbamod(109+7)` → $C_a^b \bmod(10^9+7)$ |
| LaTeX 格式 | 统一使用 `$...$` 和 `$$...$$` 正确包裹公式 |
| 节标题 | 使用标准 Markdown 标题层级（`###`、`####`） |
| 代码语言标签 | 统一为 `cpp`（实际代码为 C++ 风格） |
...
给定 n 组询问，每组询问给定两个整数 a，b，请你输出 C(a,b) mod
...
\begin{aligned}
\text{证明：} \\
&n = n_0p^0 + n_1p^1 + \dots + n_{k-1}p^{k-1} + n_kp^k \\
&m = m_0p^0 + m_1p^1 + \dots + m_{k-1}p^{k-1} + m_kp^k \\
\\
\text{则有：} \\
&(1+x)^n = (1+x)^{n_0p^0 + n_1p^1 + \dots + n_{k-1}p^{k-1} + n_kp^k} \\
&= (1+x)^{n_0p^0} \times (1+x)^{n_1p^1} \times \dots \times (1+x)^{n_{k-1}p^{k-1}} \times (1+x)^{n_kp^k} \\
&= (1+x^{p^0})^{n_0} \times (1+x^{p^1})^{n_1} \times \dots \times (1+x^{p^{k-1}})^{n_{k-1}} \times (1+x^{p^k})^{n_k} \pmod{p} \\
\\
&\because C_n^m \text{在其等式左边即为 } x^m \text{ 的系数，右边也为 } x^m \text{ 的系数} \\
\text{又} &\because m \text{的p进制：} x^m = x^{m_0p^0 + m_1p^1 + \dots + m_{k-1}p^{k-1} + m_kp^k} \\
&\therefore x^{m_kp^k} \text{项在 } (1+x^{p^k})^{n_k} \text{ 中是 } C_{n_k}^{m_k}x^{m_kp^k}, \quad x^{m_{k-1}p^{k-1}} \text{项在 } (1+x^{p^{k-1}})^{n_{k-1}} \text{ 中是 } C_{n_{k-1}}^{m_{k-1}}x^{m_{k-1}p^{k-1}} \\
&\text{同理：可得 } C_n^m = C_{n_0}^{m_0} \times C_{n_1}^{m_1} \times \dots \times C_{n_{k-1}}^{m_{k-1}} \times C_{n_k}^{m_k} \pmod{p} \\
\\
&\because n, m \text{ 是p进制数} \\
&\therefore n_0 = n \bmod p, \quad m_0 = m \bmod p \\
&\text{此时将 } n, m \text{ 在p进制下右
...
$$的面积： $$
\begin{aligned}
\text{由图可知：} S &= S_1 \cup S_2 \cup S_3 \\
&= S_1 + S_2 + S_3 \\
...
设全集 U 中元素有 n 种不同的属性，第 i 种属性称为 Pᵢ。拥有属性 Pᵢ 的元素构成集合 Sᵢ，那么：

|⋃ᵢ₌₁ⁿ Sᵢ| = Σᵢ |Sᵢ| - Σᵢ&lt;ⱼ |Sᵢ ∩ Sⱼ| + Σᵢ&lt;ⱼ&lt;ₖ |Sᵢ ∩ Sⱼ ∩ Sₖ| - ⋯ + (-1)ⁿ⁻¹ |S₁ ∩ ⋯ ∩ Sₙ|

即：

|⋃ᵢ₌₁ⁿ Sᵢ| = Σₘ₌₁ⁿ (-1)ᵐ⁻¹ Σ_{a₁&lt;⋯&lt;aₘ} |⋂ᵢ₌₁ᵐ S_{aᵢ}|
...
给定一个整数 n 和 m 个不同的质数 p₁, p₂, …, pₘ。

请你求出 1∼n 中能被 p₁, p₂, …, pₘ 中的至少一个数整除的整数有多少个。
...
int main() {
    ...
    LL res = 0;
    for (int i = 1; i &lt; 1 &lt;&lt; m; i++) {  // 1~2^m-1 这些数代表 2^m-1 个不同的项
        // ...（后续代码逻辑需补充）
    }
    return 0;
}
...
\begin{aligned}
\text{证明：}& \\
&\text{设}x\text{的二进制表示中最高一位}1\text{在第}k\text{位} \\
&\text{则}a_1,a_2,\dots,a_n\text{中，必然有一个数}a_i\text{，它的第}k\text{位为}1 \\
&\text{且一定满足：}a_i\oplus x &lt; a_i \\
&\text{若从第}i\text{堆石子中拿走}(a_i - a_i \oplus x)\text{个石子} \\
&\text{则第}i\text{堆石子还剩}a_i - (a_i - a_i \oplus x) = a_i \oplus x\text{个石子} \\
&\text{由：}a_1\oplus a_2\oplus\dots\oplus a_i \oplus\dots \oplus a_n = x \ne 0\text{可知} \\
&\text{此时：}a_1\oplus a_2\oplus\dots\oplus (a_i \oplus x) \oplus\dots \oplus a_n = x \oplus x = 0
\end{aligned}
$$
...
\begin{aligned}
\text{证明：}& \text{（反证法）} \\
&\text{假设玩家从第}i\text{堆石子拿走若干个，结果异或值仍为}0 \\
&\text{不妨设拿走后，第}i\text{堆还剩下}a'\text{个石子，且}a' &lt; a_i \\
&\text{此时仍有：}a_1\oplus a_2\oplus\dots\oplus a'\oplus\dots \oplus a_n = 0 \\
&\text{则有}(a_1\oplus a_2\oplus\dots\oplus a'\oplus\dots \oplus a_n) \oplus (a_1\oplus a_2\oplus\dots\oplus a_i\oplus\dots \oplus a_n) = a'\oplus a_i = 0 \\
&\text{即：}a' = a_i，\text{与原假设}a' &lt; a_i\text{矛盾} \\
&\text{故假设不成立}
\end{aligned}
$$
...
* 在采取最优策略的情况下，一方玩家对于偶数级台阶的操作的影响，可以被另一方玩家做相同操作的结果消去。
  ...
  * 即：对于奇数级台阶，若先手面对的局面是所有奇数级台阶石子数的异或非零，即 $a_1\oplus a_3\oplus\dots\oplus a_{n'}\ne 0$（其中 $n'$ 为不超过 $n$ 的最大奇数），则先手必胜，反之先手必败。
...
* **Mex运算：** 设$S$表示一个非负整数集合，定义$Mex(S)$为求出不属于集合$S$的最小自然数运算，例如：$S=\lbrace 0,1,2,4\rbrace$，则$Mex(S)=3$
  * **$SG$函数：** 在有向图游戏中，对于每个节点$x$，设从$x$出发共有$k$条有向边，分别到达节点$y_1,y_2\dots y_k$，定义$SG(x)$为后继节点$y_1,y_2\dots y_k$的$SG$函数值构成的集合在执行$Mex$运算的结果，即：$SG(x)=Mex(\lbrace SG(y_1),SG(y_2)\dots SG(y_k)\rbrace)$，**特别地**，整个有向图游戏$G$的$SG$函数值被定义为有向图游戏起点$s$的$SG$函数值，即 $SG(G)=SG(s)$
  * **$SG$函数的性质：**
    ```java
    * 定义终点的$SG$函数值为$0$，
    ...
    * 若$SG(i)=0$，说明走不到$0$
    ```
  * **有向图游戏的和：** 若一个游戏的每个局面都可以构成一个有向图，则对于每个有向图的$SG(G_1),SG(G_2)\dots SG(G_n)$，对所有图的$SG$进行异或运算，即可得到该有向图游戏的和，且其结果符合`Nim`游戏
...
$$
\begin{aligned} 证明：&\\\ &设x的二进制表示中最高一位1在第k位\\\ &则SG(G_1),SG(G_2),\dots,SG(G_n)中，必然有一个局面SG(G_i)，其值的第k位为1\\\ &且一定满足:SG(G_i)\oplus x\lt SG(G_i)\\\ &若将第i个局面的SG值调整为(SG(G_i)\oplus x)\\\ &由：SG(G_1)\oplus SG(G_2)\oplus\dots\oplus SG(G_i) \oplus\dots SG(G_n)=x\ne 0可知\\\ &此时：SG(G_1)\oplus SG(G_2)\oplus\dots\oplus SG(G_i)\oplus x \oplus\dots SG(G_n)=x\oplus x=0 \end{aligned}
$$
...
\begin{aligned}
\text{反证法：}& \\
& \text{假设玩家改变第 } i \text{ 个局面的 SG 值，结果仍是 } 0。 \\
& \text{不妨设 } SG(G_i) \text{ 改变后的值为 } SG(G_i)'，\text{且 } SG(G_i)' &lt; SG(G_i)。 \\
& \text{此时仍有： } SG(G_1) \oplus SG(G_2) \oplus \dots \oplus SG(G_i)' \oplus \dots \oplus SG(G_n) = 0。 \\
& \text{则有 } \left(SG(G_1) \oplus SG(G_2) \oplus \dots \oplus SG(G_i)' \oplus \dots \oplus SG(G_n)\right) \\
& = \left(SG(G_1) \oplus SG(G_2) \oplus \dots \oplus SG(G_i) \oplus \dots \oplus SG(G_n)\right) \\
& = SG(G_i)' \oplus SG(G_i) = 0。 \\
& \text{即： } SG(G_i)' = SG(G_i)，\text{与原假设 } SG(G_i)' &lt; SG(G_i) \text{ 矛盾。} \\
& \text{故假设不成立。}
\end{aligned}
$$
...
* 相比于`集合-Nim`，这里的每一堆可以变成小于原来那堆的任意大小的两堆，即`a[i]`可以拆分成`(b[i],b[j])`
  ...
  * 相当于一个局面拆分成了两个局面，由$SG$函数理论，多个独立局面的$SG$值，等于这些局面$SG$值的异或和。因此需要存储的状态就是`sg(b[i])^sg(b[j])`
...
const int N = 100 + 10; // 根据数据范围调整，ai ≤ 100
...
int sg(int x) {
    ...
    vis.clear();
    for (int i = 0; i &lt; x; i++) {
        for (int j = 0; j &lt;= i; j++) { // 规定 j 不大于 i，避免重复
            vis.insert(sg(i) ^ sg(j)); // 相当于一个局面拆分成了两个局面
        }
    }
    for (int i = 0;; i++) {
        if (!vis.count(i)) return f[x] = i; // mex 操作
    }
}
