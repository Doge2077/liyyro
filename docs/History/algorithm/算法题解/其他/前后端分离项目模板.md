...
* 首先 `Star` 并且 `Fork` 本项目到你的仓库。
    * 环境依赖：`JDK17`、`Node.js 16+`、`SpringBoot 3`、`Vue3`、`Redis 5.0.14.1`
    * 推荐使用 `IDEA` 进行构建：
        * 将 `demo-backend` 模块导入，并执行 `Maven` 构建。
...
spring:
  datasource:
    ...
    url: jdbc:mysql://127.0.0.1:3306/demo?useUnicode=true&characterEncoding=utf-8  # 数据库连接地址，数据库名称默认 demo
