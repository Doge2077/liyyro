...
```
  * 状态计算： 
```cpp
* 添加字母：添加一个字母之后变得相同，说明没有添加前，`a[N]`的前`i`个字母已经和`b[N]`的前`j - 1`个字母已经相同，即`dp[i][j] = dp[i][j - 1] + 1`
* 删除字母：删除一个字母之后变得相同，说明没有添加前，`a[N]`的前`i - 1`个字母已经和`b[N]`的前`j`个字母已经相同，即`dp[i][j] = dp[i - 1][j] + 1`
