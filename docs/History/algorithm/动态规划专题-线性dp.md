...
给定一个如下图所示的数字三角形，从顶部出发，在每一节点可以选择移动至其左下方的节点或移动至其右下方的节点，一直走到底层，要求找出一条路径，使路径上的数字的和最大。
```text
      7
     3   8
    8   1   0
...
*   **状态表示** (`dp[i][j]`):
    *   **集合**：所有从起点 `(0,0)` 走到 `(i, j)` 的路径。
    *   **属性**：这些路径数字之和的最大值。
*   **状态计算**：
    *   对于点 `(i, j)`，可以从左上方的点 `(i-1, j-1)` 或右上方的点 `(i-1, j)` 走来。
    *   从 `(i-1, j-1)` 走来：`dp[i][j] = dp[i-1][j-1] + a[i][j]`。
    *   从 `(i-1, j)` 走来：`dp[i][j] = dp[i-1][j] + a[i][j]`。
    *   集合属性为最大值，取两种方案中的较大者：`dp[i][j] = max(dp[i-1][j-1], dp[i-1][j]) + a[i][j]`。
...
地里每个道路的交叉点上都种着一株花生苗，上面有若干颗花生，经过一株花生苗就能摘走它上面所有的花生。
...
**输出格式** 对每组输入数据，输出一行，内容为Hello Kitty能摘到的最多的花生颗数。
...
*   状态表示：
    *   集合：`dp[i][j]` 表示从 `a[1][1]` 走到 `a[i][j]` 的路径的花生数目之和
    *   属性：花生数目之和的最大值
*   状态计算：
...
*   状态表示：
    *   集合：`dp[i][j]` 表示从 `a[1][1]` 走到 `a[i][j]` 的路径的通行费之和
    *   属性：通行费的最小值
*   状态计算：
    *   对于 `a[i][j]`，可以由上方的点 `a[i - 1][j]` 或左方的点 `a[i][j - 1]` 走到
    *   从 `a[i - 1][j]` 走来：`if(i > 1) dp[i][j] = dp[i - 1][j] + a[i][j]`
    *   从 `a[i][j - 1]` 走来：`if(j > 1) dp[i][j] = dp[i][j - 1] + a[i][j]`
    *   集合属性为通行费之和的最小值，取两方案 `min()`
    *   由于 `i - 1` 和 `j - 1` 可能等于 $0$，故需要将 `dp` 数组初始化为 `0x3f3f3f3f`
...
*   **状态表示：**
    ...
    *   属性：物品的总价值最大。
*   **状态计算：**
    *   线路1从左方及线路2从左方到达：`dp[k][i][j] = dp[k - 1][i - 1][j - 1] + a[i][j]` (i&gt;1, j>1)
    *   线路1从左方及线路2从上方到达：`dp[k][i][j] = dp[k - 1][i - 1][j] + a[i][j]` (i>1, j>=1)
    *   线路1从上方及线路2从左方到达：`dp[k][i][j] = dp[k - 1][i][j - 1] + a[i][j]` (i>=1, j>1)
    *   线路1从上方及线路2从上方到达：`dp[k][i][j] = dp[k - 1][i][j] + a[i][j]` (i>=1, j>=1)
...
*   **状态计算**：
    ...
    *   枚举 `j &lt; i`，若 `a[j] &lt; a[i]`，则 `dp[i] = max(dp[i], dp[j] + 1)`。
    *   最终答案为所有 `dp[i]` 的最大值。
...
*   较小的数开头的上升子序列优于较大的数开头的子序列。
*   定义数组 `q[N]`，其中 `q[i]` 表示长度为 `i` 的上升子序列中末尾元素的最小值。
*   `q` 初始为空，长度 `res` 为 0。
*   枚举每个数 `x`，对于当前数 `x`，通过二分查找找到 `q` 中第一个大于等于 `x` 的位置 `pos`。
    *   若 `pos == res`，说明 `x` 可以接在所有现有子序列之后，更新 `res++`，并将 `q[res]` 设为 `x`。
    *   否则，用 `x` 更新 `q[pos]`（因为以 `x` 结尾的长度为 `pos+1` 的子序列，其末尾元素更小）。
...
int n;
int a[N], q[N]; // q[i] 表示长度为 i 的上升子序列末尾元素的最小值

void solve() {
    cin >> n;
    for (int i = 0; i &lt; n; i++) cin &gt;> a[i];

    int res = 0; // 当前最长上升子序列长度
    for (int i = 0; i &lt; n; i++) {
        int l = 0, r = res; // 在 q[0..res) 中二分查找第一个 &gt;= a[i] 的位置
        while (l &lt; r) {
            int mid = (l + r) &gt;> 1;
            if (q[mid] >= a[i]) r = mid;
            else l = mid + 1;
        ...
        // 此时 l 是第一个 >= a[i] 的位置
        q[l] = a[i];
        if (l == res) res++; // 如果 a[i] 比所有末尾都大，则长度加一
    }
...
*   状态表示：
    *   集合：`dp[i][j]` 表示 `a` 的前 `i` 个字母和 `b` 的前 `j` 个字母的公共子序列长度。
    *   属性：最大长度。
*   状态计算：
...
*   状态表示：
    *   集合：`dp[i][j]` 表示 `a` 中的前 `i` 个字母变成 `b` 中前 `j` 个字母的操作集合
    *   属性：操作的最小次数
*   状态计算：
    *   添加字母：添加一个字母之后变得相同，说明没有添加前，`a` 的前 `i` 个字母已经和 `b` 的前 `j - 1` 个字母已经相同，即 `dp[i][j] = dp[i][j - 1] + 1`
    *   删除字母：删除一个字母之后变得相同，说明没有删除前，`a` 的前 `i - 1` 个字母已经和 `b` 的前 `j` 个字母已经相同，即 `dp[i][j] = dp[i - 1][j] + 1`
...
每组测试数据包含两行：
- 第一行是一个整数 N，代表有 N 幢建筑。
- 第二行包含 N 个不同的整数，每一个对应一幢建筑的高度 h，按照建筑的排列顺序给出。
...
队员们希望在满足上面条件的同时，尽可能多地游览景点，你能帮他们找出最多能游览的景点数吗？
...
**思想**
*   先分别求正序和倒序的最长上升子序列
    *   当 `flag != 1` 时，遍历 `dp1[N]` 和 `dp2[N]`，最大值即为 `max(dp1[i], dp2[n - i + 1])`
...
*   先分别求正序和倒序的最长上升子序列
    *   `flag != 1` 时，遍历 `dp1[N]` 和 `dp2[N]`，能排列成合唱队的最大人数为 `max(dp1[i], dp2[n - i + 1])`
...
编程帮助政府做出一些决定和拒绝申请的决定，使得在保证任意两条航线不相交的情况下，被批准的申请尽量多。
...
一个数的序列 `ai`，当 `a1 &lt; a2 &lt; … &lt; as` 的时候，我们称这个序列是上升的。
...
*   状态表示：
    ...
    *   属性：集合中的上升子序列和的最大值
*   状态计算：
    *   枚举 `a[j]` 为上升子序列的倒数第二个数，初始化为 `dp[i] = a[i]`
...
*   最多能拦截的导弹数 = 最长不上升子序列长度
*   最少需要的系统数 = 最长上升子序列长度（根据 Dilworth 定理）
...
*   状态表示：
    *   集合：`dp[i][j]` 表示 `a` 中前 `i` 个数字，`b` 中前 `j` 个数字，且当前以 `b[j]` 结尾的公共上升子序列的长度
    *   属性：公共上升子序列的长度的最大值
*   状态计算：
    *   依据公共子序列中是否包含 `a[i]`，将 `dp[i][j]` 所代表的集合划分成两个不重不漏的子集
    *   不包含 `a[i]` 的子集，最大值是 `dp[i][j] = dp[i - 1][j]`
    *   包含 `a[i]` 的子集，将该子集继续划分，依据是子序列的倒数第二个元素在 `b` 中是哪个数：
        *   子序列只包含 `b[j]` 一个数，长度是1；
