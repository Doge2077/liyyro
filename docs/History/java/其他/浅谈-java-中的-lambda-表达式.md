...
由此可见，在函数式编程的思想下，我们将功能的具体细节隐藏，将其抽象为了函数式接口，这就使得代码具有规范、稳定、可组合、高复用的特点。
...
因此，要使用接口，就**必须提供相应的实现类，或者包含实现接口的对象返回**。例如，要想使用 `List` 接口，我们可以使用实现了该接口的实现类 `ArrayList`、`LinkedList` 等，或者像上节例子一样，使用 `Arrays.asList` 的工厂方法返回了一个实现了 `List` 接口的 `ArrayList` 对象。
...
public class Main {
    public static void main(String[] args) {
        String claim = "Test number is ";
        claim = "Yeah~ The number is "; // 从 lambda 表达式引用的本地变量必须是最终变量或实际上的最终变量
        Test test = param -> claim + param;
        System.out.println(test.showTestNumber(2333));
    }
}
...
该方法是一个静态方法，但它和 `Comparator` 需要实现的方法的返回值和参数定义一模一样，因此我们直接进行方法引用：
```java
public class Main {
    public static void main(String[] args) {
        Integer[] array = new Integer[]{4, 5, 9, 3, 2, 8, 1, 0, 6};
...
public class Main {
    public static void main(String[] args) {
        Test test = new Test() {
            @Override
            public String newTest(String param) {
                return param;
            }
        };
...
经过上面的学习，相信你已经可以熟练地使用 `Lambda` 表达式了。看起来 `Lambda` 只是一种简化匿名内部类实现接口的语法糖，但实际上，它们是两种本质不同的事物：

*   匿名内部类本质是一个类，只是不需要我们显式地指定类名，编译器会自动为该类取名。
...
可以看到，`Main$1.class` 实际上就是 `Main` 类中生成的匿名内部类文件。而将其替换为 `Lambda` 表达式后，编译的结果如下：

![image-20230827224417232](https://image.itbaima.net/images/40/image-20230827222290045.png)

没有生成单独的类文件，即匿名内部类对应的是一个 `class` 文件，而 `Lambda` 表达式对应的是它所在主类的一个私有方法。
